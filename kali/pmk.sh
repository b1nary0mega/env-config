#!/bin/bash
#
# PMK (Personalize My Kali) - b1nary0mega
# Personalized for my tooling and style
#
# Forked from pimpmykali by Dewalt
# https://github.com/Dewalt-arch/pimpmykali
#
# Usage: sudo ./pmk.sh  ( defaults to the menu system )
...
check_for_root() {
    if [ "$EUID" -ne 0 ]
      then echo -e "\n\n Script must be run with sudo ./pmk.sh or as root \n"
      exit
    else
...
fix_sead_warning() {
 ...
 "$bold$redexclaim$red" WARNING "$redexclaim$bold$red"  PMK IMPACKET REMOVAL FUNCTION  "$redexclaim$bold$red" WARNING "$redexclaim$white$norm"
...
#---- begin pmk-mirrors rev 1.3.2 08.20.2021
get_mirrorlist() {
    cleanup
    fix_sources
    echo -e "\n  ${greenplus} PMK-Mirrors - kali repo mirror speedtest"
...
install_brave() {
    is_installed "gpg apt-transport-https curl"
    eval curl -fsSL https://brave-browser-apt-release.s3.brave.com/brave-core.asc | gpg --dearmor -o /usr/local/share/keyrings/brave-browser-archive-keyring.gpg
    
    eval echo "deb [signed-by=/usr/local/share/keyrings/brave-browser-archive-keyring.gpg] https://brave-browser-apt-release.s3.brave.com/ stable main" | tee /etc/apt/sources.list.d/brave-browser-release.list > /dev/null
    apt_update
...
# ascii art - DONT move
asciiart=$(base64 -d <<< ""  | gunzip )
...
pmk_menu() {
    while true; do
      ...
      echo -e "  "$bold"N - NEW VM SETUP"$reset" - Run this option if this is the first time running pmk\n"
      echo -e "  = - PMK-Mirrors                   (find fastest kali mirror. use the equals symbol = )"         # get_mirrorlist; best_ping; small_speedtest; large_speedtest; gen_new_sources; cleanup;;
      ...
      echo -e "  Please use sudo ./pmk.sh --help for additional installations/fixes\n"
      # menu selection
...
pmk_help() {
    ...
    "      --mirrors  run PMK-mirrors speedtest"
