...
if [ ! -f /opt/ohmyzsh.sh ]; then
    ...
    if [ -n "$SUDO_USER" ]; then
        ...
        sudo -u "$SUDO_USER" sed -i 's/^plugins=(git)$/plugins=(git debian ansible colorize)/' "~$SUDO_USER/.zshrc"
        sudo -u "$SUDO_USER" sed -i 's/^ZSH_THEME="robbyrussell"$/ZSH_THEME="aussiegeek"/' "~$SUDO_USER/.zshrc"
    fi
fi
